...
/*
connNum
=> init: map<seq, <payload, ACK>>
=> resp: map<seq, <payload, ACK>>

=> meta: initIP:p, respIP:p, init_numPk, resp_numPk,  
...
void handler_callback(u_char *args, const struct pcap_pkthdr* pkthdr, const u_char* packet){
   ...
   if(ntohs(etherHead -> ether_type) == ETHERTYPE_IP){
      ...
      if(ipHead -> ip_p == IPPROTO_TCP){
         ...
         //cout << "payload size is: " << dataStr.length() << endl;
         ...
         if(hasTT){
            ...
            if(((tcpHead -> th_flags) & 0x02)){
               ...
               string ConnStr = InitStr + "-" + respStr;
               metaPacket[ConnStr].push_back(connNum);
               metaPacket[ConnStr].push_back();
               metaPacket[ConnStr].push_back();
               metaPacket[ConnStr].push_back();
               metaPacket[ConnStr].push_back();
               metaPacket[ConnStr].push_back();
